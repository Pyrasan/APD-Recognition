        ...
        for r in results:
            for box in r.boxes:
                ...
                if conf > 0.65:
                    ...
                    cvzone.putTextRect(img, f'{label} {conf_txt}', (max(0, x1), max(35, y1)), scale=1, thickness=1)

    run_model(modelhelm, "Helm")
    run_model(modelrompi, "Rompi")
    run_model(modelsepatu, "Safety Boots")
    run_model(modelsarungtangan, "Sarung Tangan")
...
def confirm_detection(name, folder_path, excel_file):
    ...
    # Ubah ke set lengkap sesuai kebutuhan
    required_apd = {"Helm", "Rompi", "Safety Boots", "Sarung Tangan"}
    status = "Diizinkan" if required_apd.issubset(detected_items) else "Tidak Diizinkan"

    ext = os.path.splitext(apd_image_path)[1]
